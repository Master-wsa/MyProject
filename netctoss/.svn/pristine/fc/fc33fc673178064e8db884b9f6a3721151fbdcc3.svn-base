            					...
            					var sip=$("#sip").val()
            	      			var regsip= /^((1?\d?\d|(2([0-4]\d|5[0-5])))\.){3}(1?\d?\d|(2([0-4]\d|5[0-5])))$/
            	      				if(regsip.test(sip)==false){
            		      				...
            		      				$("#ipmsg").removeClass()
            		      				$("#ipmsg").addClass("validate_msg_long success");
            		      				$("#ipmsg").html("&nbsp")
            		      			}
            				}
            			}
	      	...
	      	$(function(){
        		$("#pwd").blur(function(){
        			...
        			if(regpwd.test(pwd)==false){
        				...
        				$("#pwdmsg").removeClass()
        				$("#pwdmsg").addClass("validate_msg_long success");
            		    $("#pwdmsg").html("&nbsp")
        			}
        		})
        		$("#pwd").focus(function(){
        		...
        		$("#repwd").blur(function(){
        			if($("#pwd").val()!=$("#repwd").val()){
        				...
        				$("#repwdmsg").removeClass()
        				$("#repwdmsg").addClass("validate_msg_long success");
		                $("#repwdmsg").html("&nbsp")
        			}
        		})
            				...
            				if(data=="yes"){
                	        	...
                	        	var regos=/[0-9a-zA-Z_]{6,18}/
                   	        	if(regos.test(osname)==false){
                   	        		...
                   	        		$("#osmsg").removeClass()
                   	        		$("#osmsg").addClass("validate_msg_long success");
		                    	    $("#osmsg").html("&nbsp")
                   	        	}
            				}
            			}
            		...
            		$.ajax({
            			...
            			success:function(data){
            				if(data.length<5){
            					$("#accidd").val(data)
            				    $("#showacc").removeClass()
            					$("#showacc").addClass("validate_msg_long success");
		                    	$("#showacc").html("&nbsp") 
            				}else{
            					$("#accidd").val()
            					$("#showacc").removeClass()
        ...
        <div id="main">
            ...
            <form id="myform" action="${pageContext.request.contextPath}/service/addService.do" method="post" class="main_form">
                ...
                <div class="input_info">
                    <input type="text" id="idc" name="idcard" class="width200" />
                ...
                <div class="input_info">
                    <input type="text"  id="accidd" name="accountid" class="width200" readonly="readonly" read onkeyup="searchAccounts(this);" />
                    <span class="required">*</span>
                ...
                <div class="input_info">
                    <input type="text" class="width200" id="sip" name="sip" />
                     <span class="required">*</span>
                   <div id="ipmsg" class=""></div>
                </div>                   
                <div class="text_info clearfix"><span>登录 OS 账号：</span></div>
                <div class="input_info">
                    <input type="text" class="width200" id="osname" name="osname" />
                    <span class="required">*</span>
                     <div id="osmsg" class=""></div> 
                </div>
                <div class="text_info clearfix"><span>密码：</span></div>
                <div class="input_info">
                    <input type="password" class="width200" id="pwd" name="pwd" />
                     <span class="required">*</span>
                  <div id="pwdmsg" class=""></div> 
                </div>
                <div class="text_info clearfix"><span>重复密码：</span></div>
                <div class="input_info">
                    <input type="password" class="width200"  id="repwd" name="pwd2" />
                     <span class="required">*</span>
