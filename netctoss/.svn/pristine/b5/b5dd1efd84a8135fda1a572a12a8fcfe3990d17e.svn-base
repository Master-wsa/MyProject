...
<html xmlns="http://www.w3.org/1999/xhtml">
        ...
        <script language="javascript" type="text/javascript">
          ...
          //删除
            function deleteBuss(finId) {
                ...
                if(r==true){
                	$.ajax({
            			url:'${pageContext.request.contextPath}/service/clearService.do',
            			type:'post',
            			data:{
            				'finId':finId,
            			},
            			success:function(data){
            				if(data=="yes"){
            				  $("#operate_result_info").html("删除成功！");
          					  ...
          					  $("#operate_result_info").delay(3000).hide(0);
          					 /*  setTimeout('select()', 1000); */
          					 if('${totalCount-(currPage-1)*8}'==1&&'${totalCount}'!=1){
          						$("#myform").attr("action","${pageContext.request.contextPath}/service/getByPageCondition.do?currPage=${currPage-1}");
               	            	$("#myform").submit();
          					 }else{
          						$("#myform").attr("action","${pageContext.request.contextPath}/service/getByPageCondition.do?currPage=${currPage}");
               	            	$("#myform").submit(); 
          					 }
            				}else{
            ...
            function setStatePause(finId) {
                     ...
                     if(r==true){
                    	 $.ajax({
                 			url:'${pageContext.request.contextPath}/service/pauseService.do',
                 			type:'post',
                 			data:{
                 				'finId':finId,
                 			},
            ...
            function setStateOpen(finId) {
                ...
                if(r==true){
                	if($("#in"+finId).val()==0){
                		...
                		$.ajax({
                 			url:'${pageContext.request.contextPath}/service/openService.do',
                 			type:'post',
                 			data:{
                 				'finId':finId,
                 			},
    ...
    <body>
        ...
        <!--主要区域开始-->
        <div id="main">
            <form id="myform" action="${pageContext.request.contextPath }/service/getByPageCondition.do" method="post">
                <!--查询-->
                ...
                <div id="data">            
                   ...
                   <c:forEach items="${busses}" var="buss" >
                    	...
                    	<input type="hidden" id="inn${currPage}" value="${currPage}" />
                   </c:forEach>
                    </table> 
        	   ...
        	   &nbsp;&nbsp;第${currPage}页/共${totalPage}页&nbsp;&nbsp;&nbsp;共${totalCount}条记录
                   </c:if>                               
                </div>                    
        </form>
        </div>
